# Source: https://github.com/FushengHao/CPEA
# Adjusted hardcoded seq_len (196) to be flexible
...
class CPEA(nn.Module):
    def __init__(self, in_dim=384, seq_len=196, dropout=0.1, class_aware_factor=2.0):
        super(CPEA, self).__init__()

        # λ in the paper: controls how strongly the class token is mixed
        # into each patch before similarity is measured.
        # Authors use 2.0; lower values keep more patch locality.
        self.class_aware_factor = class_aware_factor

        self.fc1 = Mlp(in_features=in_dim, hidden_features=int(in_dim/4), out_features=in_dim, drop=dropout)
        self.fc_norm1 = nn.LayerNorm(in_dim)

        self.fc2 = Mlp(in_features=seq_len**2, hidden_features=256, out_features=1, drop=dropout)

    def forward(self, feat_query, feat_shot, n_shot=5):
        # query: Q x n x C
        ...
        feat_query = query_image + self.class_aware_factor * query_class  # Q x L x C
        feat_shot  = support_image + self.class_aware_factor * support_class  # KS x L x C

        feat_query = F.normalize(feat_query, p=2, dim=2)
        feat_query = feat_query - torch.mean(feat_query, dim=2, keepdim=True)

        feat_shot = feat_shot.contiguous().reshape(n_shot, -1, n -1, c)  # K x S x n x C
