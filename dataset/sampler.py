# Credit: https://github.com/sicara/easy-few-shot-learning
...
class TaskSampler(Sampler):
    ...
    def episodic_collate_fn(
        ...
        # Preserve first-seen order so episodic labels stay aligned with the
        # class order implied by the sampled batch. CPEA assumes that support
        # features are grouped in this exact class order when forming logits.
        true_class_ids = list(dict.fromkeys(x[1] for x in input_data_with_int_labels))
        ...
        # Produce shot-interleaved ordering to match CPEA's expected reshape:
        #   Author's CategoriesSampler uses .t().reshape(-1) which gives
        #   [c0s0, c1s0, c2s0, ..., c0s1, c1s1, c2s1, ...] (interleaved by shot)
        #   CPEA's forward does reshape(n_shot, n_way, L, C) then mean(dim=0)
        #   which only produces correct per-class prototypes with this ordering.
        support_images = all_images[:, : self.n_shot].permute(
            1, 0, *range(2, len(all_images.shape))
        ).reshape((-1, *all_images.shape[2:]))
        query_images = all_images[:, self.n_shot :].permute(
            1, 0, *range(2, len(all_images.shape))
        ).reshape((-1, *all_images.shape[2:]))
        support_labels = all_labels[:, : self.n_shot].t().flatten()
        query_labels = all_labels[:, self.n_shot :].t().flatten()
        return (
            support_images,
            support_labels,
            query_images,
